...
PROJECT=walkthrough-operator
SHELL = /bin/bash
TAG = 0.0.1
PKG = github.com/integr8ly/walkthrough-operator
...
compile:
	go build -o=walkthrough-operator ./cmd/walkthrough-operator
...
.PHONY: uninstall
uninstall:
	-kubectl delete role walkthrough-operator -n $(NAMESPACE)
	-kubectl delete rolebinding default-account-walkthrough-operator -n $(NAMESPACE)
	-kubectl delete crd walkthroughs.integreatly.aerogear.org
	-kubectl delete namespace $(NAMESPACE)


.PHONY: create-examples
create-examples:
	-kubectl create -f deploy/examples/walkthrough.yaml -n $(NAMESPACE)
